...
**Immediate next action:** Obtain explicit user instruction on one of: authorize calibration `T007-CAL-1` by name (recommended path per A5), authorize live experiment `LX-1` (`specs/005-queue-cpu-feasibility-architecture/live-experiment-proposal.md`, `STATUS: NOT AUTHORIZED`), record a waiver per `contracts/waiver.md`, or amend Feature 004 in place after review (single-pass / unit architecture) — none of these may begin without authorization (`docs/progress/PROGRESS.md` Feature 005 T049 follow-ups).
